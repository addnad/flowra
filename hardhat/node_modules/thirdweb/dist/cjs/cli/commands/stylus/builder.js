...
async function buildStylus(spinner, secretKey) {
    ...
    try {
        ...
        const abiResult = (0, node_child_process_1.spawnSync)("cargo", ["stylus", "export-abi"], {
            encoding: "utf-8",
        });
        ...
        const abiArray = selectedInterface.abi;
        ...
        if (constructorAbi &&
            ...
            !abiArray.some((e) => e.type === "constructor")) {
...
function parseSolidityInterfaces(source) {
    const results = [];
    const ifaceRegex = /interface\s+(I?[A-Za-z0-9_]+)\s*\{([\s\S]*?)\n\}/g;
    for (let ifaceMatch = ifaceRegex.exec(source); ifaceMatch !== null; ifaceMatch = ifaceRegex.exec(source)) {
        ...
        for (let structMatch = structRegex.exec(body); structMatch !== null; structMatch = structRegex.exec(body)) {
        ...
        const resolveStructs = (sig) => {
            let resolved = sig;
            for (let i = 0; i < 10; i++) {
                let changed = false;
                for (const [sName, sTuple] of structs) {
                    const re = new RegExp(`\\b${sName}\\b(\\[\\])?`, "g");
                    const next = resolved.replace(re, (_, arr) => `${sTuple}${arr ?? ""}`);
                    if (next !== resolved) {
                        resolved = next;
                        changed = true;
                    }
        ...
        for (const stmt of statements) {
            ...
            try {
                const parsed = (0, abitype_1.parseAbiItem)(cleaned);
                abi.push(parsed);
            }
            catch {
